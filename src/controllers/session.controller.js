    ...
    try {
        passport.authenticate('jwt', { session: false }, async (err, user, info) => {
            ...
            if (!user) {
                ...
                const userBDD = await MUser.getElementByEmail(email)
                ...
                //ACA bycript no esta convirtiendo el dato password 
                //para compararlo con el dato ingresado.
                //Dps vere porque. Por ahora lo comento para continuar.                

                /*
                if (!validatePassword(password, userBDD.password)) {
                    console.log(password, userBDD.password)
...
export const registerUser = async (req, res) => {
    try {
        ...
        if (userBDD) {
            ...
            const newUser = await MUser.addElements({ first_name, last_name, email, age, password: hashPassword })
            console.log(newUser)
